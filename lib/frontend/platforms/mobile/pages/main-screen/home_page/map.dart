...
class _MapsState extends State<Maps> with TickerProviderStateMixin {
  ...
  Set<Marker> _createMarkers(MapState state) {
    ...
    if (state is MapDataLoaded) {
      ...
      for (var dangerZone in state.dangerZones) {
        ...
        _dangerZones.add(LatLng(dangerZone.latitude!, dangerZone.longitude!));
      ...
      for (var safeZone in state.safeZones) {
        _safeZones.add(LatLng(safeZone.latitude!, safeZone.longitude!));
        markers.add(
          Marker(
 ...
 Future<void> findNearestSafezone() async {
    ...
    if (widget.UserToken == 'guest') {
      print("👤 Guest user: Using Dagupan Police Station as default");

      SharedPreferences.getInstance().then((prefs) {
        prefs.setString('nearest_station_name', "Dagupan Police Station");
        print("🟡 Guest mode: Default station set to Dagupan Police Station");
      });
      return;
    }
    ...
    if (nearest != null) {
      SharedPreferences.getInstance().then((prefs) {
        prefs.setString(
            'nearest_station_name', nearest!.name ?? "Dagupan Police Station");
        print("Nearest safe zone saved to prefs: ${nearest.name}");
      });
      print("🟢 Nearest Safe Zone: ${nearest.name}");
    } else {
      print("⚠️ No valid safe zones found.");
  ...
  void _updateFloatingWidget(Widget? widget) {
    setState(() {
                    ...
                    child: Column(
                      ...
                      children: [
                        Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (_floatingWidget != null) _floatingWidget!,
                              Padding(
