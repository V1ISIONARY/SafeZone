...
class CircleBloc extends Bloc<CircleEvent, CircleState> {
  ...
  CircleBloc(this._circleImplementation) : super(CircleInitial()) {
    ...
    on<FetchCirclesEvent>((event, emit) async {
      ...
      try {
        final circles = await _circleImplementation.getUserCircles(event.userId);
        emit(CircleLoadedState(circles: circles));
      } catch (e) {
        emit(CircleErrorState(message: 'Error fetching circles: ${e.toString()}'));
      }
    });
    ...
    on<CreateCircleEvent>((event, emit) async {
      emit(CircleLoadingState());
      try {
        final circle = await _circleImplementation.createCircle(event.name, event.userId);
        emit(CircleCreatedState(circle: circle));
      } catch (e) {
        emit(CircleErrorState(message: 'Error creating circle: ${e.toString()}'));
      }
    });
    ...
    on<AddMemberEvent>((event, emit) async {
      emit(CircleLoadingState());
      try {
        await _circleImplementation.addMemberToCircle(event.circleId, event.userId);
    ...
    on<RemoveMemberEvent>((event, emit) async {
      emit(CircleLoadingState());
      try {
        await _circleImplementation.removeMemberFromCircle(event.circleId, event.userId);
        emit(CircleUpdatedState(message: 'Member removed successfully'));
      } catch (e) {
        emit(CircleErrorState(message: 'Error removing member: ${e.toString()}'));
      }
    });
    ...
    on<DeleteCircleEvent>((event, emit) async {
      ...
      try {
        ...
        emit(CircleErrorState(message: 'Error deleting circle: ${e.toString()}'));
      }
    });
  }
}
